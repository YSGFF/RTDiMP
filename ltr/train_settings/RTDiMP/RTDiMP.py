...
from ltr.models.tracking import RTDiMP
...
def run(settings):
    ...
    dataset_train = sampler.DiMPSampler([lasot_train, got10k_train, trackingnet_train, coco_train], [1,1,1,1],
                                        samples_per_epoch=20, max_gap=30, num_test_frames=3, num_train_frames=3,
                                        processing=data_processing_train)
    # dataset_train = sampler.DiMPSampler([lasot_train], [1],
    #                                     samples_per_epoch=2600, max_gap=30, num_test_frames=3, num_train_frames=3,
    #                                     processing=data_processing_train)
    ...
    net = RTDiMP.rtdimpnet(filter_size=settings.target_filter_sz, backbone_pretrained=False, optim_iter=5,
    ...
    optimizer = optim.Adam([{'params': actor.net.classifier.filter_initializer.parameters(), 'lr': 5e-5},
                            ...
                            {'params': actor.net.feature_extractor.stages[2].parameters(), 'lr': 2e-5}],
                           lr=2e-4)
